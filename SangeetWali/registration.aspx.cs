    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
       ...
       // ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Selected Date: " + dt + "');", true);
        ...
        {   //  con.Close();
            ...
            string sqlstring = "INSERT INTO regist(name,fname,mname,dob,age,address,city,contact,email,guru,photo,gender,vocals,instruments,dance,school,PEI,size,type) values (@name,@fname,@mname,@dob,@age,@address,@city,@contact,@email,@guru,@photo,@gender,@vocals,@instruments,@dance,@school,@PEI,@size,@type)";
            using (SqlCommand cmd = new SqlCommand(sqlstring, con))
            {
                ...
                cmd.Parameters.AddWithValue("age", TextBox12.Text);
                cmd.Parameters.AddWithValue("address", TextBox5.Text);
                cmd.Parameters.AddWithValue("city", TextBox6.Text);
                cmd.Parameters.AddWithValue("contact", TextBox7.Text);          
                cmd.Parameters.AddWithValue("email", TextBox8.Text);
                cmd.Parameters.AddWithValue("guru", TextBox10.Text);
                cmd.Parameters.AddWithValue("gender", DropDownList2.Text);
                cmd.Parameters.AddWithValue("photo", "~/Offer_pic/" + FileUpload1.FileName.ToString());
           
                cmd.Parameters.AddWithValue("vocals", DropDownList1.Text);
                cmd.Parameters.AddWithValue("instruments", DropDownList3.Text);
                cmd.Parameters.AddWithValue("dance", DropDownList4.Text);
                cmd.Parameters.AddWithValue("fromourinstitute", DropDownList5.Text);
                cmd.Parameters.AddWithValue("school", TextBox13.Text);
                cmd.Parameters.AddWithValue("PEI", TextBox14.Text);
                cmd.Parameters.AddWithValue("size", DropDownList6.Text);
                cmd.Parameters.AddWithValue("type", DropDownList7.Text);
                cmd.Parameters.AddWithValue("stuas_app", "yes");
